...
按论文要求，复现模型使用**原始 103 波段（不 PCA）**、11×11 patch，输入 `N×1×103×11×11`。训练协议沿用论文设定：SGD、学习率 1e-4、batch size 64、Dropout 0.5、最大 300 epoch，并启用 Early Stopping（验证 loss 连续 10 轮改善不足 0.003 时停止）。数据划分为 `fair24_6_70 + seed1442`，与 HybridSN 完全一致，仅预处理（不 PCA、patch=11）与训练协议按各论文要求。因本实验各数据集的波段数/类别数与论文原始设定（B=125、C=12）不同，两个模型在三个数据集上的实际参数量如下（论文原始设定下 Paper3DCNN=951,652、Paper3D1DCNN=225,292，已通过 `verify_models.py` 对齐验证）：

| 模型 | 论文原始（125/12） | Pavia（103/9） | Indian Pines（200/16） | Salinas（204/16） |
|---|---:|---:|---:|---:|
| Paper3DCNN | 951,652 | 771,041 | 1,566,568 | 1,599,336 |
| Paper3D1DCNN | 225,292 | 214,561 | 262,064 | 263,600 |
...
两个复现模型及其改进版在 Indian Pines、Salinas 上同样完成训练，逐类准确率如下（每列一个模型，16 类）：

**Indian Pines（16 类，测试 7,175）**：

| 类别 | Paper3DCNN | Paper3D1DCNN | 改进Paper3D1DCNN |
|---|---:|---:|---:|
| Alfalfa | 93.75% | 96.88% | **0.00%** |
| Corn-notill | 95.40% | 91.70% | 79.60% |
| Corn-mintill | 97.76% | 87.44% | 79.52% |
| Corn | 98.19% | 89.76% | 14.46% |
| Grass-pasture | 98.52% | 97.34% | 56.80% |
| Grass-trees | 99.61% | 97.65% | 98.04% |
| Grass-pasture-mowed | 70.00% | 85.00% | **0.00%** |
| Hay-windrowed | 100.00% | 100.00% | 85.37% |
| Oats | 57.14% | 28.57% | **0.00%** |
| Soybean-notill | 96.91% | 93.82% | 44.56% |
| Soybean-mintill | 97.85% | 94.65% | 87.67% |
| Soybean-clean | 92.05% | 88.92% | 12.05% |
| Wheat | 87.41% | 93.01% | 32.87% |
| Woods | 96.28% | 98.31% | 98.65% |
| Buildings-Grass-Trees-Drives | 94.07% | 88.89% | 72.22% |
| Stone-Steel-Towers | 100.00% | 100.00% | **0.00%** |

改进版在 Alfalfa、Grass-pasture-mowed、Oats、Stone-Steel-Towers 四类上准确率直接归零，Corn、Soybean-clean 分别跌至 14.46%、12.05%——这些少数类每类训练样本仅约 5–22 个，被 GAP 头与 BatchNorm 的抖动整体吞并，是 Indian Pines AA 腰斩（89.50%→47.61%）的直接来源。

**Salinas（16 类，测试 37,891）**：

| 类别 | Paper3DCNN | Paper3D1DCNN | 改进Paper3D1DCNN |
|---|---:|---:|---:|
| Broccoli_green_weeds_1 | 99.22% | 100.00% | 100.00% |
| Broccoli_green_weeds_2 | 99.96% | 100.00% | 99.77% |
| Fallow | 100.00% | 99.93% | 99.28% |
| Fallow_rough_plow | 98.46% | 98.36% | 99.69% |
| Fallow_smooth | 100.00% | 99.52% | 99.20% |
| Stubble | 100.00% | 100.00% | 99.93% |
| Celery | 99.92% | 99.84% | 99.80% |
| Grapes_untrained | 95.13% | 95.44% | 88.67% |
| Soil_vinyard_develop | 100.00% | 100.00% | 99.61% |
| Corn_senesced_green_weeds | 97.82% | 99.61% | 99.13% |
| Lettuce_romaine_4wk | 100.00% | 99.60% | 98.66% |
| Lettuce_romaine_5wk | 100.00% | 99.56% | 100.00% |
| Lettuce_romaine_6wk | 100.00% | 100.00% | 98.44% |
| Lettuce_romaine_7wk | 99.87% | 99.47% | 99.73% |
| Vinyard_untrained | 90.37% | 97.74% | 88.97% |
| Vinyard_vertical_trellis | 98.58% | 98.74% | 98.42% |

Salinas 样本量大、地块规则，三个模型逐类精度普遍较高；改进版在 Grapes_untrained（88.67%）、Vinyard_untrained（88.97%）两块大类别上相对落后，是其在 Salinas OA 下降 2.7pp 的主要来源。

![图 21　Indian Pines 改进 Paper3D1DCNN 分类图（明显退化的直观证据）](论文复现结果/各模型图/indianpines__improvedpaper3d1dcnn__classification_map.png)

![图 22　Indian Pines Paper3DCNN 混淆矩阵（少数类混淆）](论文复现结果/各模型图/indianpines__paper3dcnn__confusion_matrix.png)

![图 23　Salinas 改进 HybridSN 分类图（本组最优分类图）](论文复现结果/各模型图/salinas__improvedhybridsn__classification_map.png)

## 7. 多模型对比（HybridSN / 改进 HybridSN 与复现模型）

下表在**同一划分 `fair24_6_70`、同一 seed=1442、统一早停协议**下汇总五类模型（复现模型的原始光谱输入与 11×11 patch、HybridSN 的 PCA15 与 25×25 patch 均按各自论文要求），测试集仅评测一次。表中 HybridSN 数值来自统一的 15 项对比运行（accuracy 早停，Pavia 于第 17 轮停）；它与 §3 预处理对比实验的 99.95%（全 30 轮、无早停、best_epoch 25）相差 0.05pp，纯属早停轮数不同，非划分/数据差异。

| 模型 | 波段/patch | 参数量 | OA | AA | Kappa | 训练耗时 |
|---|---:|---:|---:|---:|---:|---:|
| 原始 HybridSN | PCA15 / 25×25 | 4,844,793 | **99.9065%** | **99.8353%** | **0.998761** | 71.5 s |
| 改进 HybridSN | PCA15 / 25×25 | **152,073** | 99.7295% | 99.2534% | 0.996415 | 239.6 s |
| Paper3DCNN | 103 / 11×11 | 771,041 | 99.0048% | 98.3692% | 0.986806 | 1454.3 s |
| Paper3D1DCNN | 103 / 11×11 | 214,561 | 98.7744% | 97.9467% | 0.983756 | 1144.2 s |
| 改进 Paper3D1DCNN | 103 / 11×11 | 201,993 | 98.0230% | 96.9256% | 0.973792 | 1698.8 s |

**对比分析**：

1. **精度排序（Pavia）**：原始 HybridSN > 改进 HybridSN > 3D-CNN > 3D-1D-CNN > 改进 3D-1D-CNN。HybridSN 以 PCA15 + 更大的 25×25 patch 取得最高精度；复现模型在原始 103 波段、较小 11×11 patch 下仍达 98.0%–99.0%，验证了 3D 卷积提取谱空联合特征的有效性。
2. **参数效率**：改进 HybridSN（152K）与 3D-1D-CNN（214K）都通过去掉参数量集中的全连接头实现约 95% 级别的削减；两者证明了”全连接分类头是参数冗余主因”这一共性结论。
3. **训练代价**：复现模型使用论文的 SGD + lr=1e-4 + 300 epoch 协议，且不做 PCA（输入 103 波段），训练耗时（>1100 s）远高于 HybridSN（≈72 s）。这提示 PCA 降维不仅能提升紧凑性，也能显著降低 3D 卷积的计算量。
4. **改进 3D-1D-CNN 是反例**：同为”BN+GAP”轻量化，在论文 3D-1D 模型上（无残差）反而比原版略降 0.76pp，与改进 HybridSN（含残差）的持平不同——改进有效性依赖基础模型结构（详见 §9 与 §8 跨数据集验证）。

## 8. 多数据集验证（Pavia / Indian Pines / Salinas）

在三个数据集上统一运行 5 类模型（同一 `fair24_6_70` + seed1442 + 早停协议），测试集仅评测一次。Indian Pines（145×145，200 有效波段，16 类，10,249 有标签像元，train/val/test = 2459/615/7175）、Salinas（512×217，204 波段，16 类，54,129 有标签像元，train/val/test = 12990/3248/37891）。完整 15 项结果：

| 数据集 | 模型 | 参数量 | OA | AA | Kappa |
|---|---|---:|---:|---:|---:|
| Pavia University | 原始 HybridSN | 4,844,793 | 99.9065% | 99.8353% | 0.998761 |
| Pavia University | 改进 HybridSN | 152,073 | 99.7295% | 99.2534% | 0.996415 |
| Pavia University | Paper3DCNN | 771,041 | 99.0048% | 98.3692% | 0.986806 |
| Pavia University | Paper3D1DCNN | 214,561 | 98.7744% | 97.9467% | 0.983756 |
| Pavia University | 改进 Paper3D1DCNN | 201,993 | 98.0230% | 96.9256% | 0.973792 |
| Indian Pines | 原始 HybridSN | 4,845,696 | 99.1220% | 97.0909% | 0.989995 |
| Indian Pines | 改进 HybridSN | 152,528 | **99.2195%** | **97.7416%** | **0.991103** |
| Indian Pines | Paper3DCNN | 1,566,568 | 96.6411% | 92.1842% | 0.961708 |
| Indian Pines | Paper3D1DCNN | 262,064 | 93.8258% | 89.4950% | 0.929516 |
| Indian Pines | 改进 Paper3D1DCNN | 202,168 | 72.9895% | 47.6129% | 0.687398 |
| Salinas | 原始 HybridSN | 4,845,696 | 99.9683% | 99.9201% | 0.999647 |
| Salinas | 改进 HybridSN | 152,528 | **99.9868%** | **99.9680%** | **0.999853** |
| Salinas | Paper3DCNN | 1,599,336 | 97.4347% | 98.7081% | 0.971430 |
| Salinas | Paper3D1DCNN | 263,600 | 98.5669% | 99.2368% | 0.984051 |
| Salinas | 改进 Paper3D1DCNN | 202,168 | 95.8433% | 98.0814% | 0.953746 |

**跨数据集分析**：

1. **改进 HybridSN 三处与原始 HybridSN 持平或略优**：Indian Pines 99.22% vs 99.12%（+0.10pp）、Salinas 99.99% vs 99.97%（+0.02pp）、Pavia 99.73% vs 99.91%（−0.18pp），而参数量仅为后者的 3.1%（152K vs 4.8M）。BN+残差+GAP 的轻量化在 HybridSN 上成立。
2. **Paper3DCNN 与 3D-1D-CNN 无一致赢家**：少样本的 Indian Pines 上 3D-CNN 明显更优（96.64% vs 93.83%，+2.8pp，靠大 FC 头），大样本的 Salinas 上 3D-1D-CNN 反超（98.57% vs 97.43%）。
3. **改进 Paper3D1DCNN 三处全部退化，是明确反例**：Indian Pines 暴跌 20.8pp（AA 仅 47.6%），Pavia −0.76pp、Salinas −2.7pp。同为"BN+GAP"，因无残差且直接作用脆弱的 3D-1D 结构，mini-batch 统计量在少样本/类极不均衡下抖动大，GAP 头又抹平空间位置信息（见 §9）。
4. **Salinas 训练耗时异常值**：改进 Paper3D1DCNN 的 15473s 含笔记本夜间休眠约 3 小时，实际约 2900s，仅影响墙钟时间、不影响精度。

![图 24　五模型 × 三数据集的 OA/AA/Kappa 对比](论文复现结果/对比指标.png)

![图 25　参数量（对数轴）与 OA 的权衡](论文复现结果/参数量精度权衡.png)

![图 26　各实验训练总耗时（含 Salinas 改进版休眠异常值）](论文复现结果/训练耗时.png)

## 9. 进一步模型创新（选做）：改进 3D-1D-CNN 的负结果

在复现 3D-1D-CNN 基础上加入 **BatchNorm + 全局平均池化分类头**（`ImprovedPaper3D1DCNN`，无残差），结构见 3.7.4 节。参数量理论值约 **202,000**（较原始 3D-1D-CNN 的 214,561 减少约 6%），因为 3D-1D-CNN 本已轻量、末尾全连接较小，GAP 的收益有限，BN 是主要新增项。

**最终结果（三数据集，均为负）**：

| 数据集 | 原始 3D-1D-CNN OA | 改进版 OA | Δ |
|---|---:|---:|---:|
| Pavia | 98.7744% | 98.0230% | −0.75pp |
| Indian Pines | 93.8258% | **72.9895%** | **−20.84pp** |
| Salinas | 98.5669% | 95.8433% | −2.72pp |

**失败原因**：与改进 HybridSN（含残差）不同，该模型**无残差连接**，且 BatchNorm 直接作用于参数量极少、样本极不均衡下的 3D-1D 结构——mini-batch 统计量抖动大，GAP 头又抹平空间位置信息，对少数类（Alfalfa/Oats 每类训练样本仅约 20 个）是灾难性的（AA 0.8950→0.4761 腰斩），并跑满 300 轮仍未触发 loss 早停。**结论：BN+GAP 轻量化策略不能脱离基础模型结构照搬——改进 HybridSN 因保留残差 + 2D 卷积结构而成立，论文 3D-1D 模型则否。** 该负结果如实记录，不粉饰。
...
3. 正式训练日志的首几轮与最优轮截图；
4. 本文全部结果图（图 1–图 26），并在图下注明 run 名称与 seed；复现实验的 60 张各模型图与 3 张汇总对比图位于 `report/论文复现结果/`（`各模型图/` 与根目录）；
5. 指标与性能表（不只贴图片）；
6. 改进对比表与误差分析表；
